    ...
    if (!emailError && !passwordError && !confirmPasswordError) {
      try {
        ...
        if (response.ok) {
          ...
          if (errorData.email) {
            setEmailError(errorData.email[0]); // Assuming the backend sends errors in an array
          }
          if (errorData.new_password) {
            setPasswordError(errorData.new_password[0]); // Assuming the backend sends errors in an array
          }
